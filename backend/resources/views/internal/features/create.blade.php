...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-surface shadow-sm border border-gray-200 rounded-lg p-6">
            <form method="POST" action="{{ route('internal.features.store') }}">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" id="slug" class="input-field" value="{{ old('slug') }}"
                            placeholder="Auto-generated (e.g. monthly_invoices)">
                        <p class="text-xs text-text-muted mt-1">Used in code to check permission:
                            <code>$user->can('monthly_invoices')</code>
                        </p>
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="input-field">
                            <option value="limit" {{ old('type') == 'limit' ? 'selected' : '' }}>Limit (Count based)</option>
                    ...
                    <div>
                        ...
                        <input type="number" name="default_limit" id="default_limit" class="input-field" required
                            value="{{ old('default_limit', 0) }}" min="-1">
                        <p class="text-xs text-text-muted mt-1">Use <code>-1</code> for Unlimited. For Boolean, 0 = Off, 1 =
                            On.</p>
