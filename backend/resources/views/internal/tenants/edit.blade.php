...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-surface shadow-sm border border-gray-200 rounded-lg p-6">
            <form method="POST" action="{{ route('internal.tenants.update', $business->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" class="input-field" required
                            value="{{ old('name', $business->name) }}" placeholder="e.g. Acme Corp">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="mobile" id="mobile" class="input-field"
                                value="{{ old('mobile', $business->mobile) }}" placeholder="e.g. +91 9876543210">
                            @error('mobile') <p class="text-sm text-danger mt-1">{{ $message }}</p> @enderror
                        </div>

                        <!-- Website -->
                        <div>
                            <label for="website" class="block text-sm font-medium text-text-muted mb-1">Website
                                (Optional)</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- GSTIN -->
                        <div>
                            <label for="gstin" class="block text-sm font-medium text-text-muted mb-1">GSTIN
                                (Optional)</label>
                            <input type="text" name="gstin" id="gstin" class="input-field uppercase"
                                value="{{ old('gstin', $business->gstin) }}" placeholder="22AAAAA0000A1Z5">
                        ...
                        <div>
                            ...
                            <input type="text" name="pan" id="pan" class="input-field uppercase"
                                value="{{ old('pan', $business->pan) }}" placeholder="ABCDE1234F">
                    ...
                    <div class="border-t border-gray-100 pt-4 mt-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="bank_name" class="block text-sm font-medium text-text-muted mb-1">Bank
                                    Name</label>
                                <input type="text" name="bank_name" id="bank_name" class="input-field"
                                    value="{{ old('bank_name', $business->bank_name) }}" placeholder="HDFC Bank">
                            </div>
                            <div>
                                <label for="ifsc_code" class="block text-sm font-medium text-text-muted mb-1">IFSC
                                    Code</label>
                                <input type="text" name="ifsc_code" id="ifsc_code" class="input-field uppercase"
                                    value="{{ old('ifsc_code', $business->ifsc_code) }}" placeholder="HDFC0001234">
                            </div>
                            <div class="md:col-span-2">
                                <label for="account_number" class="block text-sm font-medium text-text-muted mb-1">Account
                                    Number</label>
                                <input type="text" name="account_number" id="account_number" class="input-field"
                                    value="{{ old('account_number', $business->account_number) }}" placeholder="50100...">
                            </div>
                        </div>
                    </div>
