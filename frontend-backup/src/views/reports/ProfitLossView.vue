      ...
      <div class="sm:flex sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-500">
            Period: <span class="font-medium text-gray-900">{{ formatDate(startDate) }}</span> to <span
              class="font-medium text-gray-900">{{ formatDate(endDate) }}</span>
          </p>
        </div>
        <div class="mt-4 sm:mt-0 grid grid-cols-1 sm:grid-cols-3 gap-4 items-end w-full sm:w-auto">
          <div>
        ...
        <div class="bg-white overflow-hidden shadow rounded-lg">
          <div class="px-4 py-5 sm:p-6">
            ...
            <dd class="mt-1 text-3xl font-semibold"
              :class="reportData.net_profit >= 0 ? 'text-indigo-600' : 'text-red-600'">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
          ...
          <ul role="list" class="divide-y divide-gray-200">
            <li v-for="account in reportData.income.accounts" :key="account.id"
              class="px-4 py-4 sm:px-6 flex justify-between items-center">
              <span class="text-sm font-medium text-gray-900 truncate pr-4">{{ account.name }}</span>
              <span class="text-sm text-gray-500 whitespace-nowrap">{{ formatCurrency(account.amount) }}</span>
            </li>
            <li v-if="reportData.income.accounts.length === 0"
              class="px-4 py-4 sm:px-6 text-center text-gray-500 text-sm">
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
          ...
          <ul role="list" class="divide-y divide-gray-200">
            <li v-for="account in reportData.expenses.accounts" :key="account.id"
              class="px-4 py-4 sm:px-6 flex justify-between items-center">
              <span class="text-sm font-medium text-gray-900 truncate pr-4">{{ account.name }}</span>
              <span class="text-sm text-gray-500 whitespace-nowrap">{{ formatCurrency(account.amount) }}</span>
            </li>
            <li v-if="reportData.expenses.accounts.length === 0"
              class="px-4 py-4 sm:px-6 text-center text-gray-500 text-sm">
