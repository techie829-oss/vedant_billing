<template>
  <div class="min-h-screen bg-gray-50 flex flex-col">
    ...
    <div class="flex flex-1">
      ...
      <div v-if="mobileMenuOpen" class="lg:hidden fixed inset-0 z-50 flex flex-col justify-end"
        @click.self="mobileMenuOpen = false">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-black/40" @click="mobileMenuOpen = false"></div>
        <!-- Sheet -->
        <div class="relative bg-white rounded-t-2xl shadow-2xl z-10">
          <!-- Handle -->
          ...
          <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
            <div
              class="h-9 w-9 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold text-sm shrink-0">
              {{ userInitials }}</div>
            <div class="min-w-0">
              <p class="text-sm font-semibold text-gray-800 truncate">{{ authStore.user?.name || 'User' }}</p>
              <p class="text-xs text-gray-500 truncate">{{ authStore.activeBusiness?.name || '' }}</p>
            </div>
            <button @click="handleLogout"
              class="ml-auto text-xs text-red-500 font-semibold shrink-0 py-1 px-2 rounded-lg hover:bg-red-50">Logout</button>
          </div>
          <!-- Menu Grid -->
          <div class="p-3 grid grid-cols-4 gap-2">
            <router-link v-for="item in moreMenuItems" :key="item.to" :to="item.to" @click="mobileMenuOpen = false"
              class="flex flex-col items-center gap-1 py-3 rounded-xl transition-colors"
              :class="$route.path.startsWith(item.to) ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-50'">
          ...
          <!-- Settings Row -->
          <div class="px-3 pb-5 flex gap-2 border-t border-gray-100 pt-2">
            <router-link v-if="canManageBusiness" to="/settings/business" @click="mobileMenuOpen = false"
              class="flex-1 flex items-center justify-center gap-1.5 px-2 py-2.5 rounded-xl text-xs font-medium text-gray-600 bg-gray-50">
            ...
            <router-link to="/businesses" @click="mobileMenuOpen = false"
              class="flex-1 flex items-center justify-center gap-1.5 px-2 py-2.5 rounded-xl text-xs font-medium text-gray-600 bg-gray-50">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
// Items for the bottom-sheet More menu grid
const moreMenuItems = [
  { to: '/quick-note', label: 'Fast Note', icon: 'M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10' },
  { to: '/customers', label: 'Customers', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0z' },
  ...
  { to: '/quotations', label: 'Estimates', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
  { to: '/credit-notes', label: 'Credit Notes', icon: 'M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l2.333-1.75L10 21l2.333-1.75L15 21l1.667-1.75L19 21z' },
  { to: '/cashbook', label: 'Cashbook', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
  ...
  { to: '/invoice-scans', label: 'Scans', icon: 'M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z' },
  { to: '/delivery-challans', label: 'Delivery', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h5.586a1 1 0 00.707-.293l5.414-5.414a1 1 0 00.293-.707V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
  { to: '/debit-notes', label: 'Debit Notes', icon: 'M12 6v6m0 0v6m0-6h6m-6 0H6' },
]
