<template>
    <div class="space-y-6">
        <div class="bg-white shadow sm:rounded-lg">
            ...
            <!-- Summary Cards -->
            <div v-if="totals"
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-6 border-b border-gray-200">
                ...
                <div class="bg-green-50 p-4 rounded-lg">
                    ...
                    <div class="mt-1 text-2xl font-semibold text-green-900">{{ formatCurrency(totals.total_amount) }}
                    </div>
                </div>
                <div class="bg-yellow-50 p-4 rounded-lg">
                    <div class="text-sm font-medium text-yellow-600">Balance Due</div>
                    <div class="mt-1 text-2xl font-semibold text-yellow-900">{{ formatCurrency(totals.balance_due) }}
                    </div>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg">
                    <div class="text-sm font-medium text-gray-600">Tax Collected</div>
                    <div class="mt-1 text-2xl font-semibold text-gray-900">{{ formatCurrency(totals.tax_collected) }}
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="sm:hidden space-y-4 px-4 pb-4">
                    <div v-if="loading && !invoices.length" class="text-center py-4 text-gray-500">Loading...</div>
                    <div v-else-if="!invoices.length" class="text-center py-4 text-gray-500">No invoices found.</div>
                    <div v-for="inv in invoices" :key="inv.id"
                        class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                <div class="font-bold text-gray-900">{{ inv.invoice_number }}</div>
                                <div class="text-xs text-gray-500">{{ inv.date }}</div>
                            </div>
                        ...
                        <div class="text-sm text-gray-900 mb-2 truncate">{{ inv.party?.name || 'Unknown' }}</div>
                        <div class="flex justify-between items-center border-t border-gray-100 pt-2 mt-2">
                            <span class="text-sm font-medium text-gray-900">{{ formatCurrency(inv.grand_total) }}</span>
                            <router-link :to="`/invoices/${inv.id}`"
                                class="text-sm text-indigo-600 font-medium hover:text-indigo-900">View
                                Details</router-link>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr v-else-if="!invoices.length">
                                <td colspan="5" class="px-6 py-4 text-center text-gray-500">No invoices found for this
                                    period.</td>
                            </tr>
                            <tr v-for="inv in invoices" :key="inv.id">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ inv.party?.name ||
                                    'Unknown' }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">{{
                                    formatCurrency(inv.grand_total) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
