<template>
    <div class="space-y-6">
        <div class="bg-white shadow sm:rounded-lg">
             <div class="px-6 py-5 border-b border-gray-200 sm:px-6 flex justify-between items-center bg-gray-50">
                <div class="flex items-center space-x-4">
                     <div class="relative flex items-start">
                        <div class="flex h-6 items-center">
                          <input id="low-stock" type="checkbox" v-model="filters.low_stock_only" @change="loadReport" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                        </div>
                 ...
                 <div class="text-sm text-gray-500 bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm">
                    Valuation: <span class="font-bold text-gray-900 text-lg ml-2">{{ formatCurrency(summary.total_valuation) }}</span>
                </div>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Sales Price</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr v-for="product in products" :key="product.id">
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-500">{{ product.hsn_code ? `HSN: ${product.hsn_code}` : '' }}</div>
                            </td>
