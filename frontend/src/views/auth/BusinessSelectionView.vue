  ...
  <AuthLayout>
    ...
    <div v-else class="space-y-4">
      ...
      <div class="grid gap-4">
        <button
          v-for="business in auth.userBusinesses"
          :key="business.id"
        ...
        >
          <div class="flex-shrink-0 h-10 w-10 rounded-lg bg-indigo-50 flex items-center justify-center group-hover:bg-indigo-600 transition-colors duration-200">
          ...
          <div class="ml-4 flex-1">
            ...
            <p class="text-xs text-gray-500 flex items-center">
              ...
              ></span>
              {{ business.currency }} • {{ business.status === 'active' && (!business.subscriptions?.length || business.subscriptions[0].status === 'active') ? 'Active' : 'Inactive (Cancelled)' }}
            </p>
          </div>
