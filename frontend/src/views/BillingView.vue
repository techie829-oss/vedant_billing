<template>
  <AppLayout>
    <div class="mb-8 flex justify-between items-end">
        ...
        <div v-if="currentSubscription" class="bg-indigo-50 text-indigo-700 px-4 py-2 rounded-lg text-sm font-medium border border-indigo-100">
            ...
            <div v-if="currentSubscription" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div v-if="currentSubscription.plan">
                    <div class="flex items-center space-x-3 mb-2">
                        <span class="text-3xl font-bold" :class="currentSubscription.status === 'active' ? 'text-gray-900' : 'text-red-600'">
                    ...
                    <p class="text-sm mb-6" :class="currentSubscription.status === 'active' ? 'text-gray-500' : 'text-red-500 font-medium'">
                    ...
                    <button @click="scrollToPlans" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
                        Change Plan &rarr;
                    </button>
                </div>
                ...
                <div v-else class="text-red-600">
                    <h3 class="text-xl font-bold mb-2">Plan Not Found</h3>
                     <p class="text-sm mb-4">The plan associated with this subscription seems to be invalid or deleted.</p>
                    <button @click="scrollToPlans" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
                ...
                <div class="space-y-4 border-l border-gray-100 pl-8 md:pl-8 sm:border-l-0 sm:pl-0">
                    ...
                    <div v-if="currentSubscription.plan">
                        ...
                        <div v-if="currentSubscription.plan.features.every((f: any) => f.pivot.limit < 0)" class="text-sm text-green-600 flex items-center">
...
<span class="font-medium text-gray-900">₹{{ currentSubscription && currentSubscription.plan ? currentSubscription.plan.price : '0.00' }}</span>
                    </div>
                 </div>
            </div>
        </div>
        ...
        <div id="available-plans" class="pt-8 border-t border-gray-200">
             ...
             <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                >
                    <div v-if="isCurrentPlan(plan.id)" class="absolute top-0 right-0 bg-indigo-500 text-white text-xs font-bold px-3 py-1 rounded-bl-lg">
                        Current Plan
                    </div>
                    ...
                    <div class="p-6 flex-1">
                        ...
                        <p class="mt-4 text-sm text-gray-500">{{ plan.description || 'Perfect for growing businesses.' }}</p>

                        <ul class="mt-6 space-y-4">
                            <li v-for="feature in plan.features" :key="feature.id" class="flex items-start">
                                <!-- Included Feature (Limit > 0 or -1) -->
                                <svg v-if="feature.pivot.limit !== 0" class="flex-shrink-0 h-5 w-5 text-green-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM6.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4a1 1 0 00-1.414-1.414L9 11.586 6.707 9.293z" clip-rule="evenodd" />
                                </svg>
                                ...
                                <span class="ml-3 text-sm" :class="feature.pivot.limit === 0 ? 'text-gray-400' : 'text-gray-700'">
                                    {{ feature.name }} 
                                    <span v-if="feature.pivot.limit > 0 && feature.type === 'limit'" class="text-gray-500">({{ feature.pivot.limit }})</span>
                                    <span v-else-if="feature.pivot.limit < 0" class="text-gray-500">(Unlimited)</span>
                                </span>
                            </li>
                        </ul>
                    </div>
